...
const Navbar = () => {
    ...
    return (
        <motion.nav
            ...
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || isOpen ? 'glass shadow-lg' : 'bg-transparent'
                }`}
        >
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="lg:hidden absolute top-20 left-0 right-0 h-[calc(100vh-5rem)] bg-white/95 backdrop-blur-xl border-t border-white/20 shadow-xl overflow-y-auto"
                    >
                        <div className="flex flex-col items-center justify-center min-h-full py-8 space-y-6">
                            {navLinks.map((link, index) => (
                                <motion.a
                                    key={link.name}
                                    href={link.href}
                                    className="text-2xl font-medium text-slate-800 hover:text-violet-600 transition-colors"
                                    initial={{ opacity: 0, y: 20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    transition={{ delay: index * 0.1 }}
                            ...
                            <motion.a
                                href="#contact"
                                className="gradient-btn text-lg px-8 py-3 rounded-full mt-4 shadow-xl"
